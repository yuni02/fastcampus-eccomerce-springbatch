...
public class BatchStepExecutionListener implements StepExecutionListener {
  ...
  public ExitStatus afterStep(StepExecution stepExecution) {
    ...
    // Step 실행 횟수 증가
    manager.getStepExecutionCounter()
        .labels(jobName, stepName, status.toString())
        .inc();
    ...
    if (stepExecution.getStartTime() != null && stepExecution.getEndTime() != null) {
      ...
      manager.getStepDurationGauge()
          .labels(jobName, stepName)
          .set(durationSeconds);
    }

    // Step 실패 횟수 기록
    if (status == BatchStatus.FAILED) {
      manager.getStepFailureCounter()
          .labels(jobName, stepName)
          .inc();
    }
